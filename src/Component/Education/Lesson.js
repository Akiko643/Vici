        ...
        >
            ...
            <div className='h-210 w100 flex-col justify-between mt-10'>
                {elements.map((el, index) => (
                    <div className="flex flex-row items-center" key={index}><div className="too"><p>{index+1}</p></div>{el}</div>
                ))}
            </div>
            <div className='w100 flex-center mt-20 mb-20'>
                <div className='w-130 h-45 text-center bradius-10 b-secondary flex-center c-white pointer' onClick={() => history.push(location.pathname+ "/" + id)}>Start learn</div>
            </div>
        </div>
    );
}
