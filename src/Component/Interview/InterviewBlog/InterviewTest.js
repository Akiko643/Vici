...
export const InterviewTest = () => {
    ...
    const { data:doc } = useDoc(`/content/contents/Interview/${match.params.interviewId}`);
    ...
    useEffect(async () => {
        const query = await firebase.firestore().collection(`/content/contents/Interview`).orderBy('createdAt', 'desc').limit(3);
    ...
    return (
        <div>
            ...
            <div className="interview__container pa-vw-10 pr">
                <img src={Back} className="arrow pointer" onClick={() => history.push('/interview')}/>
